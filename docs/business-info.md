# BuildRight Construction LLC: Business Info

Source: intake questionnaire provided by the owner. Fields left blank in the
original doc are marked `(not provided)`. Update this file as more info comes
in (photos, reviews, licensing, service-area edge cases, etc.).

## 1. Business Information

- Business name: BuildRight Construction LLC
- Owner: Jose Mendoza
- Phone: 708-822-8984
- Email: brco.chi@gmail.com
- Business address: (not provided)
- Years in business: 9 years
- Business hours: (not provided)
- Google Business Profile: (not provided)
- Facebook / Instagram / other social: (not provided)

## 2. About the Business

BuildRight Construction LLC is a locally owned general construction and
remodeling company serving Chicago and the surrounding suburbs. Founded by
Jose Mendoza after a 10-year career as an emergency room nurse, BuildRight
was built on the same values that guided him in healthcare: attention to
detail, clear communication, problem-solving, and genuine care for people.
They specialize in interior renovations, bathrooms, kitchens, basements,
flooring, decks, fences, and custom improvement projects. The goal is to
combine quality craftsmanship with thoughtful design to create beautiful,
functional spaces while treating every client and home with respect.

**Experience:** ~9 years of remodeling/construction experience, starting in
2017 when Jose bought his first property and began renovating/repairing it
hands-on, later expanding into professional client work.

**Typical customers:** Homeowners, landlords, and real estate investors
throughout Chicago and the surrounding suburbs, from small repairs to
full-house rebuilds.

**Differentiator:** "BuildRight": doing things correctly, attention to
detail, never cutting corners. Honest recommendations, clear communication,
dependable service, respect for the client's home. Positioned as a partner
throughout the project (material/color/finish selection, budget-fitting
ideas), personally involved from first conversation through final walkthrough.

## 3. Services

**Full list:**
- Whole-home renovations, rehabs, and rebuilds
- Kitchen remodeling
- Bathroom remodeling and shower conversions
- Basement remodeling and finishing
- Garage remodeling and finishing
- Room conversions and layout modifications
- Demolition and debris removal
- Framing, insulation, drywall installation, and drywall repair
- Interior and exterior painting
- Luxury vinyl plank, laminate, and tile flooring installation
- Carpet and existing-floor removal
- Tile showers, floors, walls, and backsplashes
- Finish carpentry, trim, baseboards, and shoe molding
- Custom built-ins, shelving, cabinets, and storage solutions
- Interior, exterior, and storm-door installation
- Window installation and replacement
- Deck construction, resurfacing, and repair
- Wood and composite fence installation and repair
- Gates, railings, and exterior carpentry
- Lighting and recessed-light installation
- Switches, dimmers, outlets, and minor electrical upgrades
- Toilets, vanities, faucets, and other plumbing-fixture installations
- Minor plumbing repairs and water-line installations
- Appliance, microwave, range hood, and water-filtration-system installation
- Water-intrusion diagnostics and water-damage repairs
- Flat-roof and exterior leak repairs
- Playset and specialty-product assembly
- General home repairs and small improvement projects
- Design guidance, material selection, color coordination, project planning,
  and trade coordination

BuildRight can manage projects of nearly any size, from a single repair or
installation to a complete property transformation. Specialized electrical,
plumbing, and other regulated work is coordinated with qualified
professionals when required.

**Main/biggest services:** Whole-home renovations and rehabs, kitchen
remodeling, bathroom remodeling, basement finishing, and deck/fence
construction. Also flooring, carpentry, drywall, painting, and related
improvements as part of larger projects. Specialty is complete
transformations from planning through completion (though smaller jobs are
accepted too).

**Services NOT offered:** None specified.

**Free estimates:** Yes, free initial consultations/estimates for most
projects. Services requiring extensive diagnostic work, detailed planning,
or specialized inspections may involve a fee, always discussed with the
customer in advance.

**Financing:** No formal financing currently offered. Structured payment
schedules based on project size/progress are available instead.

## 4. Service Area

- Primary: Chicago and surrounding Chicagoland suburbs, including Cicero,
  Oak Park, Skokie, Morton Grove, Bolingbrook, and neighboring communities in
  the western, northern, northwestern, and southwestern suburbs.
- Projects outside the primary area may be considered depending on
  size/scope.
- Typical travel radius: ~40 miles from Chicago; farther for larger
  renovations/full-property projects depending on scope and timeline.
- Excluded areas: None specified.

## 5. Projects / Previous Work

Real photos received 2026-09-17 and added to the Projects page and homepage,
organized by project type (kitchen, bathroom, basement, deck, fence,
whole-home renovation). See `public/images/projects/` and
`src/data/projects.ts`. Still awaiting: per-project details (location, exact
dates/timeline, before-and-after pairs, client name/permission to use).

## 6. Customer Reviews

(not provided: Google reviews link, other review pages, testimonials, and
highlighted reviews still needed)

## 7. License / Insurance / Credentials

(not provided: license number, licensed/insured/bonded status,
certifications, awards, professional memberships)

## 8. Team

(not provided: owner name/title/bio/photo, other team members, company
photo)

## 9. Estimates & Contact

- Free estimates: Yes
- How customers should request an estimate: Phone, text, email, website
  form, or any of the above.
- Property visit before estimate: (not provided)
- Info normally needed from customer before estimating: (not provided)
- Anything else customers should know before contacting: (not provided)

## 10. Business Photos & Materials

(not provided: logo, team/company photos, owner photo, truck/vehicle
photos, jobsite photos, before/after photos, finished project photos,
business cards, flyers/brochures, yard signs, uniforms, other marketing
materials)

## 11. Anything Else

(not provided)
